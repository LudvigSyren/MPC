...
class SystemModel:
    ...
    def discretize_system_matrices(self, sample_time: float) -> None:
        ...
        # print(f"A=\n{self.Ad}")
        # print(f"B=\n{self.Bd}")
    def dynamics_rocket(self, x, u):
        normalization_u = np.diag([self.env.cfg.main_engine_thrust, self.env.cfg.side_engine_thrust, self.env.cfg.max_nozzle_angle])
        un = normalization_u @ u
        x_ = np.copy(x)
        dx= np.zeros(6)
        dx[0] = x_[2]
        dx[1] = x_[3]
        dx[2] = (un[1]*np.cos(x_[4])-un[0]*np.sin(un[2]+x_[4])) / self.mass
        dx[3] = (un[1]*np.sin(x_[4])+un[0]*np.cos(un[2]+x_[4])) / self.mass-self.gravity
        dx[4] = x_[5] 
        dx[5] = (-un[1]*self.l2-un[0]*self.l1*np.sin(un[2])) / self.inertia
        return dx
    def step_rocket(self,x,u,dt):
        ddt = dt/10
        x_ = np.copy(x)
        for i in range(10): 
            x_ += ddt*self.dynamics_rocket(x_,u)
        return x_
